...
```{r otu rarefaction}
otu_t <- t(otu_sup0.01)
otu1_t <- matrix(as.numeric(unlist(otu_t)),
              ncol = ncol(otu_t),
              nrow = nrow(otu_t))
rownames(otu1_t) <- rownames(otu_t)
colnames(otu1_t) <- colnames(otu_t)

otu_t_rarefied <- as.data.frame(rrarefy(otu1_t, min(rowSums(otu1_t))))

otu_t_rarefied <- t(otu_t_rarefied)
otu_t_rarefied <- matrix(as.numeric(unlist(otu_t_rarefied)),
              ncol = ncol(otu_t_rarefied),
              nrow = nrow(otu_t_rarefied))

rownames(otu_t_rarefied) <- rownames(otu)
colnames(otu_t_rarefied) <- colnames(otu)
```
...
otu_phy <- otu_table(otu_t_rarefied, taxa_are_rows = TRUE)
