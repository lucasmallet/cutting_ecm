...
otu_samples_out <- otu[, !(colnames(otu) %in% not_samples_kept)] # suppressing from dataset 
...
hist(reads_sum_per_sample$rowSums.otu_t.) # rarefying based on the min sample depth (here 419)
...
# statistical test
lm_seq_depth <- lm(reads_sum_per_sample ~ physeq_001kept@sam_data$pool + physeq_001kept@sam_data$site)
anova(lm_seq_depth) 
...
clean_pq(remove_empty_samples = TRUE,remove_empty_taxa = T, clean_samples_names = F)
