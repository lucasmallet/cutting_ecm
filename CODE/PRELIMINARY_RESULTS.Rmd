...
- Halasen edge study: spatial dependence among samples inside a same plot (distant from 7.5 meters from each other) but independence of others plots 
  -> Should we consider the factor "plot" as random variable or just the concerned ones ? If so, how to do it ? GLMM, paired-ANOVA ? 
...
Hellinger transformation is sqrt (reads of species / total reads in sample) ie sqrt (relative abudances)
 ...
 - Zero occurence of OTUs or no reads in some samples -> leads to a biais:
  ...
  -- not considering them (ie erasing samples with 0 from dataset): min richness = 1 & N(samples) reduced -> no significant differences in mean comparison tests"
...
# hala
miss_sam_clust_hala <- anti_join(x = sam_hala[,1:7], y = hala_ecm@sam_data[,1:7]) # exclude treatment_edge column because we changed factor values for this column
...
# per site (rarefied)
sum(hala_ecm@otu_table)
sum(hala5_ecm@otu_table)
sum(fanga_ecm@otu_table)

pourc_hala_ecm <- sum(hala_ecm@otu_table)/sum(hala@otu_table)*100; pourc_hala_ecm # 29.2% ecm hala
...
colnames(hala_sam)
summary(hala_sam[,c(2,5:8)])
table(hala_ecm@sam_data$plot, hala_ecm@sam_data$treatment)
table(sam_hala$plot, sam_hala$treatment)

# lacking :
## after clustering:
### 2 (n=1); 9 (n=2) -> N=3 missing in initial sample_data matrix (64 samples in design)
length(sam_hala$sample_id)

## after clean_pq (empty samples):
### 2 (n=1); 9 (n=2); 3 (n=3); 8 (n=1) -> N= 7 
length(hala_ecm@sam_data$sample_id) # N= 7 missing after bioinformatic analysis 
# result: 3 (n=3); 8 (n=1) 

table(hala_ecm_12@sam_data$plot, hala_ecm_12@sam_data$treatment)
table(hala_ecm_18@sam_data$plot, hala_ecm_18@sam_data$treatment)

summary(hala_ecm@sam_data$treatment_edge)
summary(as.factor(hala@sam_data$treatment_edge))
...
div_hala <- vegan::diversity(x = t(hala_ecm@otu_table),
                             index = "shannon")
rich_hala <- vegan::specnumber(x= t(hala_ecm@otu_table))
summary(rich_hala)
summary(div_hala)

df_div_hala <- data.frame(hala_ecm@sam_data, div_hala, rich_hala)

stde_hala <- tapply(df_div_hala$div_hala, df_div_hala$treatment_edge, stde) # if needed for tables
median_hala <- tapply(df_div_hala$div_hala,df_div_hala$treatment_edge, median)
min_hala <- tapply(df_div_hala$div_hala,df_div_hala$treatment_edge, min)
max_hala <- tapply(df_div_hala$div_hala,df_div_hala$treatment_edge, max)

boxplot(div_hala ~ hala_ecm@sam_data$treatment_edge)
boxplot(rich_hala ~ hala_ecm@sam_data$treatment_edge)

# model all
str(df_div_hala)
lm_div <- lm(rich_hala ~ specification_value, data = df_div_hala) 
par(mfrow=c(2,2))
plot(lm_div)
par(mfrow=c(1,1))
hist(lm_div$res)
...
boxplot(rich_hala ~ hala_ecm@sam_data$specification_value)

# model all addition ## TO RESOLVE AND CHOOSE
str(df_div_hala)
lm_div <- lm(rich_hala ~ orient + stream, data = df_div_hala) 
par(mfrow=c(2,2))
plot(lm_div)
par(mfrow=c(1,1))
hist(lm_div$res)
...
## 3.2.1 alpha
```{r rarecurve}
hala_rrcurve_groups1 <- phyloseq.extended::ggrare(hala_grp_12, step = 10, color = "treatment_edge",plot=F, se=T)+ 
  facet_wrap(~year)+
  labs(x = "Group size",
       y =  "OTU richness",
...
hala_rrcurve_groups2 <- phyloseq.extended::ggrare(hala_grp_18, step = 10, color = "treatment_edge",plot=F, se=T)+ 
  facet_wrap(~year)+
  labs(x = "Group size",
       y =  "OTU richness",
       title = "rarefaction curves by treatment",
...
genus_nb_hala <- as.factor(hala_ecm@tax_table[,5]) %>% droplevels() %>% levels() %>% length() %>% print()
## OTU Genus count (0;1) by treatment
hala_ecm_ggplot1 <- phyloseq::plot_bar(as_binary_otu_table(hala_ecm), x = "treatment_edge") 
hala_ecm_ggplot1 <- ggplot(hala_ecm_ggplot1$data, aes(fill= fct_reorder(as.factor(Genus), Abundance), x=treatment_edge, y = Abundance))+
...
hala_ecm_ggplot2 <- phyloseq::plot_bar(hala_ecm, x = "treatment_edge") 
hala_ecm_ggplot2 <- ggplot(hala_ecm_ggplot2$data, aes(fill= fct_reorder(as.factor(Genus), Abundance), x=treatment_edge, y = Abundance))+
  geom_bar(stat="identity", position = "stack")+
  scale_fill_manual(values = funky(genus_nb_hala))+
  facet_grid(~year, scale="free")+
...
hala_ecm_ggplot3 <- phyloseq::plot_bar(as_binary_otu_table(hala_ecm), x = "treatment_edge") 
hala_ecm_ggplot3 <- ggplot(hala_ecm_ggplot3$data, aes(fill= fct_reorder(as.factor(OTU_ID), Abundance), x=treatment_edge, y = Abundance))+
  geom_bar(stat="identity", position = "stack") +
...
hala_box1 <- phyggplot( aes(x=name, y=value, fill=name)) +
    geom_boxplot() +
...
hala_box2 <- boxplot_abundance(d= hala_ecm, x='treatment_edge', y='otu_table',
   line='subject')
```
...
hala_ecm_ordi_mdsjacc <- plot_ordination(hala_ecm, hala_ecm_ordi_mdsjacc, type="samples", color="treatment_edge",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
  scale_color_manual(values = c("CC_EDG" = "orange",
...
hala_ecm_ordi_mdsbray <- plot_ordination(hala_ecm, hala_ecm_ordi_mdsbray, type="samples", color="treatment_edge",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
  scale_color_manual(values = c("CC_EDG" = "orange",
...
hala_ordi_specvalues_mdsjacc <- ordinate(as_binary_otu_table(hala_ecm), method = "MDS", dist="jaccard")
hala_ecm_ordi_mdsjacc <- plot_ordination(hala_ecm, hala_ordi_specvalues_mdsjacc, type="samples", color="specification_value",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
