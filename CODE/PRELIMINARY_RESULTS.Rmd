...
rowSums(table_postbioinf) ## after clean_pq (empty samples):
## 2 (n=1); 9 (n=2); 3 (n=3); 8 (n=1) -> N= 4 
n_postbionf <- length(ecm_phy@sam_data$sample_id) %>% print() # N = 4 "0 samples" after bioinformatic analysis 
### plots: 3 (CC, n=3); 8 (FOR, n=1) 
...
ecmtax_df[, c("Phylum", "Class", "Family", "Genus", "Fruitbody_type_template")] %>%
  gather(variable, value) %>%
  ggplot()+
  geom_bar(aes(x = variable, y = ..count../sum(..count..)*5, fill = value), color= " black", position = "stack", show.legend = FALSE)+
...
div_hala <- vegan::diversity(x = t(ecm_phy@otu_table),
                             index = "shannon")
rich_hala <- vegan::specnumber(x= t(ecm_phy@otu_table))
summary(rich_hala)
summary(div_hala)

df_div_hala <- data.frame(ecm_phy@sam_data, div_hala, rich_hala)
...
boxplot(div_hala ~ ecm_phy@sam_data$treatment_edge)
boxplot(rich_hala ~ ecm_phy@sam_data$treatment_edge) # same as hill0 (down) -> good

# violin plot + jitter

bxplt_1 <- df_div_hala %>% ggplot(aes(x = treatment_edge, y = rich_hala, fill =  treatment_edge))+
  geom_boxplot() +
  geom_jitter() +
  facet_grid(~ year) +
  theme(legend.position="none", plot.title = element_text(size=11)) +
  ggtitle("Richness in OTU for cutting position") +
  xlab("Position from cutting")+
  scale_fill_manual(values = coltreatment)
bxplt_1

summary(as.factor(df_div_hala$treatment_edge))

labs.data <- paste(gsub("/","-\n",row.names(table(data$Variable2,data$Variable3)),fixed=T),"\n(N=",table(data$Variable2,data$Variable3),")",sep="")

ggplot(data, aes(x=Variable2, y=Variable1, color=Variable2,fill= Variable2)) +
geom_point(position=position_jitter(width=0.15), alpha=0.8,size=1.5) + ylim(0,150) +
geom_boxplot(fill=NA, outlier.size=NA,lwd=1) + guides(fill=FALSE) + scale_x_discrete(labels=labs.data) +
ylab("Variable1") + geom_violin(alpha=0.3,trim=FALSE) + facet_grid(. ~ Variable3,scales = "free_x", space="free_x") + theme_light() +
theme(legend.position="none") + theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank()) + theme(axis.title.x=element_blank())
...
# model all
colnames(df_div_hala)
df_div_hala[, c(3:13)] <- df_div_hala[, c(3:13)] %>% lapply(as.factor) %>% data.frame()
str(df_div_hala)

# assumption test for model
hist(rich_hala) # not normal : poisson distribution lambda = 2 

glmm_rich_pois <- lme4::glmer(rich_hala ~ specification_value + treatment_edge + (specification_value| plot) + (treatment_edge| plot), 
                      family = poisson(link = "log"),
                      data = df_div_hala) # glmm : poisson transformation on richness

lm_rich <- lme4::lmer(rich_hala ~ specification_value + treatment_edge + (specification_value| plot) + (treatment_edge| plot), 
                      data = df_div_hala) # linear model without transformation

par(mfrow=c(1, 3))
hist(rich_hala) # graphical distrib
hist(residuals(glmm_rich_pois))
hist(residuals(lm_rich))
shapiro.test(rich_hala) # shapiro tests
shapiro.test(residuals(glmm_rich_pois))
shapiro.test(residuals(lm_rich))

par(mfrow=c(2, 2))
plot(glmm_rich)
qqnorm(residuals(glmm_rich_pois))
qqline(residuals(glmm_rich_pois))
...
```
```{r TREATMENT}
# fixed variables : treatment + year
# random variables: plot ()
```
...
ecm_phy_ggplot1 <- ggplot(ecm_phy_ggplot1$data, aes(fill= fct_reorder(as.factor(Genus), Abundance), x=treatment_edge, y = Abundance))+
...
ecm_phy_ggplot2 <- ggplot(ecm_phy_ggplot2$data, aes(fill= fct_reorder(as.factor(Genus), Abundance), x=treatment_edge, y = Abundance))+
  geom_bar(stat="identity", position = "stack")+
  scale_fill_manual(values = funky(genus_nb_hala))+
  facet_grid(~year, scale="free")+
