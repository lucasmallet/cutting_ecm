...
df_div <- data.frame(ecm_phy@sam_data, div, rich)
colnames(df_div)
df_div[, c(3:12)] <- lapply(df_div[, c(3:12)], as.factor) 
str(df_div)
...
glmm_want <- lme4::glmer(rich ~ treatment_edge * year + stream +
                           (1 | plot + orient),
                         family = poisson(link = "log"),
                         data = df_div)

glmm_richpois <- lme4::glmer(rich ~ treatment_edge * year + (1 | plot), 
                      family = poisson(link = "log"),
                      data = df_div) # glmm : poisson transformation on richness

lm_rich <- lme4::lmer(rich ~ treatment_edge + year + (1 | plot),
                      data = df_div)

glm <- glm(rich ~ treatment_edge + year, 
                      family = poisson(link = "log"),
                      data = df_div)

lm <- lm(rich ~ treatment_edge + year, data = df_div)
...
shapiro.test(rich_) # shapiro tests
