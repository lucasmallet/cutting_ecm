...
read_distrib <- plot_read_distribution(ecm_phy, groups = "site", 
                                       plot.type = "density")
read_distrib

# rarecurve by samplings
ecm_phy_rrcurve <- phyloseq.extended::ggrare(ecm_phy, step = 10, color = "year", label="sample_id",plot=F, se=T) 
ecm_phy_rrcurve + facet_wrap(~site)
...
### 3.2.1 alpha
```{r rarecurve}
hala_rrcurve <- phyloseq.extended::ggrare(hala_ecm, step = 10, color = "specification", label="sample_id",plot=F, se=T) 
hala_rrcurve + facet_wrap(~year)
###
hala_rrcurve_groups <- microbiomeutilities::plot_alpha_rcurve(hala_ecm, index="observed", 
                       subsamples = c(2,100,250,500,750,1000),
                       lower.conf = 0.3, 
                       upper.conf = 0.7,
                       group="specification",
                       label.color = "brown3",
                       label.size = 3,
                       label.min = TRUE,
                       type = "CI") # no min depth of 2 reads/sample (cf 1% relative keep)
hala_rrcurve_groups
```
```{r circle graph: genus distribution among treatments}
circle_hala_ecm <- circle_pq(physeq = hala_ecm, 
                              fact = "specification", 
                              taxa = "Genus", 
                              min_prop_mod=0.001,
                              min_prop_tax=0.01,
                              row_col = c("red","orange","darkgreen"),
                              grid_col= 1,
                              add_nb_seq = T)
circle_hala_ecm
```
...
hala_hill_treatment <- hill_pq(physeq=hala_ecm_t, variable = "specification", color_fac = NA, letters = T) 
hala_hill_treatment[[1]]+ 
...
hala_hill_treatment[[2]]+ labs(title="Hill 1 : position from CC", subtitle="Hagadalen_edge")
hala_hill_treatment[[3]]+ labs(title="Hill 2 : position from CC", subtitle="Hagadalen_edge")
...
plot_ordination(hala_ecm, hala_ecm_ordi_mdsjacc, type="samples", color="specification",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
...
plot_ordination(hala_ecm, hala_ecm_ordi_mdsbray, type="samples", color="specification",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
  scale_color_manual(values = c("red","orange","darkgreen"))+
...
```{r rarecurve}
hala5_rrcurve <- phyloseq.extended::ggrare(hala5_ecm, step = 10, color = "treatment", label="sample_id",plot=F, se=T) 
hala5_rrcurve
###
hala5_rrcurve_groups <- microbiomeutilities::plot_alpha_rcurve(hala5_ecm, index="observed", 
                       subsamples = c(2,100,250,500,750,1000),
                       lower.conf = 0.3, 
                       upper.conf = 0.7,
                       group="treatment",
                       ...
                       label.min = TRUE,
                       type = "CI") # no min depth of 2 reads/sample (cf 1% relative keep)
hala5_rrcurve_groups
```
```{r circle graph: genus distribution among treatments}
circle_hala5_ecm <- circle_pq(physeq = hala5_ecm, 
                              fact = "treatment", 
                              taxa = "Genus", 
                              min_prop_mod=0.001,
                              min_prop_tax=0.02,
                              row_col = c("red","darkgreen","orange"),
                              grid_col= 1,
                              add_nb_seq = T)
circle_hala5_ecm
```
...
```{r rarecurve}
fanga_rrcurve <- phyloseq.extended::ggrare(fanga_ecm, step = 10, color = "treatment", label="sample_id",plot=F, se=T) 
fanga_rrcurve + facet_wrap(~year)
###
fanga_rrcurve_groups <- microbiomeutilities::plot_alpha_rcurve(fanga_ecm, index="observed", 
                       subsamples = c(2,100,250,500,750,1000),
                       lower.conf = 0.3, 
                       upper.conf = 0.7,
                       group="treatment",
...
circle_fanga_ecm <- circle_pq(physeq = fanga_ecm, 
                              ...
                              min_prop_mod=0.001,
                              min_prop_tax=0.02,
                              row_col = c("red","darkgreen","brown","orange"),
                              grid_col= 1,
                              add_nb_seq = T)
```
```{r plotting tests}
