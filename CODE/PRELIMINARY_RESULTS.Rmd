...
```
```{r hill}
hala5_hill_treatment <- hill_pq(physeq = hala5_ecm_t, variable = "treatment", color_fac = NA, letters = T) 
hala5_hill0 <- hala5_hill_treatment[[1]]+ 
  labs(title="Hill 0 : treatment", subtitle="Hagadalen5")+
  coord_cartesian(xlim=c(0,16))+
  scale_x_continuous(name="Hill_0", breaks=seq(0,16,by=2))+
...
hala5_hill1 <- hala5_hill_treatment[[2]]+ 
  labs(title="Hill 1 : treatment", subtitle="Hagadalen5")+
  coord_cartesian(xlim=c(0,12))+
  scale_x_continuous(name="Hill_1", breaks=seq(0,12,by=2))+
  scale_y_continuous(name="treatment", breaks=c(1,2,3))+
  theme(axis.text.x=element_text(color="coral4", size=8, angle=0))
...
hala5_hill2 <- hala5_hill_treatment[[3]]+ 
  labs(title="Hill 2 : treatment", subtitle="Hagadalen5")+
  coord_cartesian(xlim=c(0,10))+
  scale_x_continuous(name="Hill_2", breaks=seq(0,10,by=2))+
  scale_y_continuous(name="treatment", breaks=c(1,2,3))+
  theme(axis.text.x=element_text(color="coral4", size=8, angle=0))
...
```{r rarecurve}
fanga_rrcurve <- phyloseq.extended::ggrare(fanga_ecm, step = 10, color = "treatment", label="sample_id",plot=F, se=T)  + facet_wrap(~year)
fanga_rrcurve
###
...
```
```{r hill fanga}
fanga_hill_treatment <- hill_pq(physeq = fanga_ecm_t, variable = "treatment", color_fac = NA, letters = T) 
...
fanga_ecm_ordi_mdsjacc <- plot_ordination(fanga_ecm, fanga_ecm_ordi_mdsjacc, type="samples", color="treatment",label="sample_id")+
  ...
  facet_wrap(~year)+
  # color and elipse
  scale_color_manual(values = c("red","darkgreen","orange","green3"))+
  stat_ellipse(type="t", level=0.70)+
  # legend
  guides(color=guide_legend(title="position from CC"))+
...
# number of genus
as.factor(hala_ecm@tax_table[,5]) %>% droplevels() %>% levels() 
ggvenn_pq(hala_ecm, fact="year", taxonomic_rank = "Genus") # to be resolved -> but interesting figure

hala_rrcurve # rarefaction curve for samples between 2012 and 2018
circle_pq(physeq = hala_ecm, 
                              fact = "specification", 
                              taxa = "Genus", 
                              min_prop_mod=0.001,
...
fanga_rrcurve # rarefaction curve for samples between 2012 and 2018
circle_pq(physeq = fanga_ecm, 
                              fact = "treatment", 
                              taxa = "Genus", 
                              min_prop_mod=0.001,
                              min_prop_tax=0.01,
                              row_col = c("red","darkgreen","orange","green3"),
                              grid_col= 1,
                              add_nb_seq = T) # distribution circle sample - taxonomy 
...
hala5_rrcurve # rarefaction curve for samples
circle_pq(physeq = hala5_ecm, 
                              fact = "treatment", 
                              taxa = "Genus", 
                              min_prop_mod=0.001,
