...
Hellinger transformation is sqrt (reads of species / total reads in sample) ie sqrt (relative abudances)
 ...
 - Zero oCUTurence of OTUs or no reads in some samples -> leads to a biais:
...
length(row.names(sam_controls_na)) # every control and NA are suppressed from the dataset (because 0 oCUTurence of clusters)
...
all_mdsbray_hellinger <- plot_ordination(physeq_hellinger, all_mdsbray_hellinger, type="samples", color="site",label="sample_id")+
  ...
  # legend
  guides(color=guide_legend(title="position to CUT"))+
...
all_mdsbray_rarefied <- plot_ordination(physeq, all_mdsbray_rarefied, type="samples", color="site",label="sample_id")+
  ...
  stat_ellipse(type="t", level=0.70)+
  # legend
  guides(color=guide_legend(title="position from CUT"))+
  ggtitle(label = "MDS of samples from Halasen_edge by year", subtitle="presence-absence (brayard distance)")
...
table12 <- table12[, plot_list] %>% print()
...
table18_missing["CUT", "3"] <- 2 # check manual 
...
N12_missing <- rowSums(table12_missing) %>% print()
N18_missing <- rowSums(table18_missing) %>% print()
...
phy_ggplot1 <- ggplot(phy_ggplot1$data, aes(fill= fct_reorder(as.factor(primary_lifestyle), Abundance), x = factor(treatment_edge, levels = order_level), y = Abundance))+
  ...
  labs(title="OTU Genus count (0;1) by position from CUT and year")+
  xlab("position from CUT")+
...
# not possible to put in random effect a variable that is NA for levels of other variable (eg stream with EDG from treatment)
...
glmm_rich <- lme4::glmer(rich ~ treatment_edge * year +
                         ...
                         data = df_div) 
summary(glmm_rich)
pair_glmm_rich <- emmeans(glmm_rich, ~ treatment_edge, type = "response")
...
## intercept = EDG + 2012 + EDG:2012, necessarily significant because model no null (0)
## CUT and FOR are S=/= of EDG independently of year     NB: (S=/= means significantly different)
## year 2018 is =/= from 2012 independently of treatment
## CUT:2018 (ie in interaction) is =/= from EDG:2012 -> CUT and EDG difference do vary through years with an observed "inversment" where richness of the edges decrease through time when richness of middle is increasing.
## FOR:2018 is =/= from EDG:2012 -> FOR and EDG difference do not vary through years (a global decrease for both of treatments is observed)

# summary interpretation of correlations:

anova(glmm, test = "LR") # order influence significance intercept (EDG select as intercept, alphabetically or 1st row of df.)
...
hala_rrcurve_groups1 <- phyloseq.extended::ggrare(yr_12, step = 10, color = "treatment_edge",plot=F, se=T)+ 
  ...
  geom_vline(xintercept = min(rowSums(yr_12@otu_table)), color = "coral4")+
  labs(x = "Group size",
       y =  "OTU richness",
  ...
  annotate("text", x = 340, y = 4, label = str_wrap("n = 331", width = 20), size = 3.5, angle=0, color = "coral4", hjust = 0)+
...
hala_hill0 <- hala_hill_treatment[[1]]+ 
  ...
  scale_y_continuous(name="Distance from CUT", breaks=c(1,2,3))+
  scale_color_manual(values = c("CUT" = "darkred",
                                "EDG" = "orange",
                                "FOR" = "darkgreen"))+
...
hala_hill1 <- hala_hill_treatment[[2]]+ 
  ...
  scale_y_continuous(name="Distance from CUT", breaks=c(1,2,3))+
  scale_color_manual(values = c("EDG" = "orange",
                                "CUT" = "darkred",
                                "FOR" = "darkgreen"))+
  theme(axis.text.x=element_text(color="coral4", size=8, angle=0))
hala_hill1
...
hala_hill2 <- hala_hill_treatment[[3]]+ 
  labs(title="Hill 2 : Distance from CUT", subtitle="Hagadalen_edge")+
...
# taxa plot
ecm_phy_mdsbray_tax <- ordinate(ecm_phy, method = "MDS", dist="bray")
ecm_phy_mdsbray_tax <- plot_ordination(ecm_phy, ecm_phy_mdsbray_tax, type = "taxa", color="treatment_edge",label = "OTU_ID")+
  geom_point(size=2)+
  # color and elipse
  scale_color_manual(values = coltreatment_raref)+
  stat_ellipse(type="t", level=0.70)+
...
hala_ordi_specvalues_mdsjaCUT <- ordinate(as_binary_otu_table(ecm_phy), method = "MDS", dist="jaCUTard")
ecm_phy_ordi_mdsjaCUT <- plot_ordination(ecm_phy, hala_ordi_specvalues_mdsjaCUT, type="samples", color="specification_value",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
  ...
  # legend
  guides(color=guide_legend(title="position from CUT"))+
  ggtitle(label = "MDS of samples from Halasen_edge by year", subtitle="presence-absence (jaCUTard distance)")
ecm_phy_ordi_mdsjaCUT
```
...
treat <- ifelse(grepl("EDG", ht_count$yr_treat), "EDG",
                         ifelse(grepl("CUT", ht_count$yr_treat), "CUT", "FOR")) # nested ifelse
...
## 3.1 stat test SY
```{r DOWNSTREAM EFFECT }
CUT_stream <- filter(df_div, treatment_edge %in% "CUT") %>% arrange(treatment_edge) 
...
hala_rrcurve_groups1 <- phyloseq.extended::ggrare(yr_12, step = 10, color = "treatment_edge",plot=F, se=T)+ 
  ...
  scale_color_manual(values = c("EDG" = "orange",
                                "CUT" = "darkred",
                                "FOR" = "darkgreen"))+
  scale_fill_manual(values = c("EDG" = "orange",
                                "CUT" = "darkred",
                                "FOR" = "darkgreen"))
...
hala_hill0 <- hala_hill_treatment[[1]]+ 
  labs(title="Hill 0 : position from CUT", subtitle="Hagadalen_edge")+
  coord_cartesian(xlim=c(0,16))+
  scale_x_continuous(name="Hill_0", breaks=seq(0,16,by=2))+
  scale_y_continuous(name="Position from CUT", breaks=c(1,2,3))+
  scale_color_manual(values = c("CUT" = "darkred",
                                "EDG" = "orange",
                                "FOR" = "darkgreen"))+
  theme(axis.text.x=element_text(color="coral4", size=8, angle=0))
...
hala_hill1 <- hala_hill_treatment[[2]]+ 
  labs(title="Hill 1 : position from CUT", subtitle="Hagadalen_edge")+
  coord_cartesian(xlim=c(0,12))+
  scale_x_continuous(name="Hill_1", breaks=seq(0,12,by=2))+
  scale_y_continuous(name="Position from CUT", breaks=c(1,2,3))+
  scale_color_manual(values = c("EDG" = "orange",
...
hala_hill2 <- hala_hill_treatment[[3]]+ 
  labs(title="Hill 2 : position from CUT", subtitle="Hagadalen_edge")+
  coord_cartesian(xlim=c(0,10))+
  scale_x_continuous(name="Hill_2", breaks=seq(0,10,by=2))+
  scale_y_continuous(name="Position from CUT", breaks=c(1,2,3))+
  scale_color_manual(values = c("EDG" = "orange",
...
ecm_phy_ggplot3 <- ggplot(ecm_phy_ggplot3$data, aes(fill= fct_reorder(as.factor(OTU_ID), Abundance), x=treatment_edge, y = Abundance))+
  ...
  scale_fill_manual(values = funky(otu_nb_hala))+
  facet_grid(~year, scale="free")+
  labs(title="OTU Genus count (0;1) by position from CUT and year", subtitle = "Halasen_edge")+
  xlab("position from CUT")+
  guides(fill=guide_legend(title="Genus"))+
  theme(axis.text.x=element_text(color="coral4", size=8, angle=20))
ecm_phy_ggplot3
...
```{r multivariate representation}
ecm_phy_ordi_mdsjaCUT <- ordinate(as_binary_otu_table(ecm_phy), method = "MDS", dist="jaCUTard")
ecm_phy_ordi_mdsjaCUT <- plot_ordination(ecm_phy, ecm_phy_ordi_mdsjaCUT, type="samples", color="treatment_edge",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
  scale_color_manual(values = c("EDG" = "orange",
...
```{r ordination}
hala5_ecm_ordi_mdsjaCUT  <- ordinate(hala5_ecm, method = "MDS", dist="bray")
hala5_ecm_ordi_mdsjaCUT  <- plot_ordination(hala5_ecm, hala5_ecm_ordi_mdsjaCUT, type="samples", color="treatment",label="sample_id")+
  geom_point(size=2)+
  # color and elipse
...
hala5_ecm_ordi_mdsjaCUT

hala5_ecm_ordi_mdsbray  <- ordinate(as_binary_otu_table(hala5_ecm), method = "MDS", dist="jaCUTard")
...
hala5_ecm_ordi1 <- ordinate(as_binary_otu_table(hala5_ecm), method = "NMDS", dist="jaCUTard",k=20)
...
fanga_ecm_only_CUT <- subset_samples(fanga_ecm, fanga_ecm@sam_data$treatment=="clearcut") %>% clean_physeq(remove_empty_samples = T,remove_empty_taxa = T)

print(fanga_ecm_only_CUT@otu_table)
which(fanga_ecm_only_CUT@otu_table>200,arr.ind = T) %>% rownames() 
fanga_ecm_only_CUT@tax_table[c("c10", "c21"),6] # 2 super abundant species 
```
```{r maybe a contamination for a sample?}
fanga_ecm_only_CUT_2012 <- subset_samples(fanga_ecm, fanga_ecm@sam_data$year=="2012" & fanga_ecm@sam_data$treatment=="clearcut") %>% clean_physeq(remove_empty_samples = T,remove_empty_taxa = T)
print(fanga_ecm_only_CUT_2012@sam_data)
print(fanga_ecm_only_CUT_2012@otu_table) # c10 in both samples, c21 in another different one... no specific contamination it seems
...
```{r ordination}
fanga_ecm_ordi_mdsjaCUT <- ordinate(as_binary_otu_table(fanga_ecm), method = "MDS", dist="jaCUTard")
fanga_ecm_ordi_mdsjaCUT <- plot_ordination(fanga_ecm, fanga_ecm_ordi_mdsjaCUT, type="samples", color="treatment",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
  scale_color_manual(values = c("clearcut" = "darkred",
  ...
  # legend
  guides(color=guide_legend(title="position from CUT"))+
  ggtitle(label = "MDS of samples from fangamon by year", subtitle="presence-absence (jaCUTard distance)")
fanga_ecm_ordi_mdsjaCUT
...
# every control and NA suppressed (ie 0 oCUTurence) 
## 8 PCR neg => good control,
...
sac_f_raw # cluster size aCUTumulation curve from all fungi clusters
```
```{r hala edge}
# design
table(hala_sam$plot,hala_sam$treatment)
...
ecm_phy_ordi_mdsjaCUT # MDS jaCUTard (0;1)
...
# hill numbers
fanga_hill0  # OTU richness # CUT 2018 huge OTU richness for a single sample (can be seen also in distribution plots)
...
fanga_ecm_ordi_mdsjaCUT # MDS jaCUTard (0;1)
...
hala5_ecm_ordi_mdsjaCUT # MDS jaCUTard (0;1)
