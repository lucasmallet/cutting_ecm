...
# post biomol design
table_postbiomol <- table(sam_hala$treatment_edge, sam_hala$plot) # post biomol
...
# post bioinf design
table_postbioinf <- table(ecm_phy@sam_data$treatment_edge, ecm_phy@sam_data$plot) # post bioinf
...
table12 <- table(ecm_12@sam_data$treatment_edge, ecm_12@sam_data$plot) %>% as.data.frame.matrix()
table12 <- table12[, plot_list] %>% print() # not working bc not all forest plot are present anymore after resampling

table18 <- table(ecm_18@sam_data$treatment_edge, ecm_18@sam_data$plot) %>% as.data.frame.matrix()
...
# variation 
mean12 <- aggregate(df_div12$rich, by = list(df_div12$treatment_edge), FUN = mean) %>% print()

sd12 <- aggregate(df_div12$rich, by = list(df_div12$treatment_edge), FUN = sd) %>% print()

mean18 <- aggregate(df_div18$rich, by = list(df_div18$treatment_edge), FUN = mean) %>% print()
sd18 <-  aggregate(df_div18$rich, by = list(df_div18$treatment_edge), FUN = sd) %>% print()
...
phy_ggplot1 <- phyloseq::plot_bar(as_binary_otu_table(phy), x = "treatment_edge") 

phy_ggplot1 <- ggplot(phy_ggplot1$data, aes(fill= fct_reorder(as.factor(primary_lifestyle), Abundance), x = factor(treatment_edge, levels = order_level), y = Abundance))+
...
# plotting 
bxplt_comp <- df_div_relat %>% ggplot(aes(x = factor(treatment_edge, levels = order_level), y = richness, fill =  primary_lifestyle))+
...
bxplt_relat <- ecm_abun_df %>% ggplot(aes(x = factor(treatment_edge, levels = order_level), y = ecm_abun, fill = treatment_edge))+
  geom_boxplot(width = 0.4, color = "black") +
...
# variation 
sd12_prop <- aggregate(ecm_abun_df12$ecm_abun, by = list(ecm_abun_df12$treatment_edge), FUN = sd) %>% print()
sd18_prop <-  aggregate(ecm_abun_df18$ecm_abun, by = list(ecm_abun_df18$treatment_edge), FUN = sd) %>% print()
mean(ecm_abun_df18$ecm_abun[ecm_abun_df18$treatment_edge == "FE"])

mean12_prop <- aggregate(ecm_abun_df12$ecm_abun, by = list(df_div12$treatment_edge), FUN = mean) %>% print()
mean18_prop <- aggregate(ecm_abun_df18$ecm_abun, by = list(df_div18$treatment_edge), FUN = mean) %>% print()
```
```{r MODEL }
# https://stats.stackexchange.com/questions/31569/questions-about-how-random-effects-are-specified-in-lmer
## for constructing model with random effects
...
# GLMM richness 
glmm_rich <- lme4::glmer(rich ~ treatment_edge * year +
                           (1 | plot),
...
summary(glmm_rich)
pair_glmm_rich <- emmeans(glmm_rich, ~ treatment_edge * year, type = "response")
pairs(pair_glmm_rich)
tab_model(glmm_rich, show.df = TRUE)

# GLMM richness 2012
glmm_rich12 <- lme4::glmer(rich ~ treatment_edge +
                           (1 | plot),
                         family = poisson(link = "log"),
                         data = df_div12) 
...
lrtest(glmm_rich12)
pair_glmm_rich12 <- emmeans(glmm_rich12, ~ treatment_edge, type = "response")
pairs(pair_glmm_rich12)

# GLMM richness 2018
glmm_rich18 <- lme4::glmer(rich ~ treatment_edge +
                           (1 | plot),
                         family = poisson(link = "log"),
                         data = df_div18) 
...
summary(glmm_rich18)
lrtest(glmm_prop18)
pair_glmm_rich18 <- emmeans(glmm_rich18, ~ treatment_edge, type = "response")
pairs(pair_glmm_rich18)

# GLMM prop
glmm_prop <- glmmTMB::glmmTMB(ecm_abun ~ treatment_edge * year +
                        (1 | plot),
                        family = beta_family(link = "logit"),
                        data = ecm_abun_df) 
car::Anova(glmm_prop, type = "III")
summary(glmm_prop)
pair_glmm_prop <- emmeans(glmm_prop, ~ treatment_edge * year, type = "response")
pairs(pair_glmm_prop)

# GLMM prop 2012
glmm_prop12 <- glmmTMB::glmmTMB(ecm_abun ~ treatment_edge +
                        (1 | plot),
                        family = beta_family(link = "logit"),
                        data = ecm_abun_df12)
...
summary(glmm_prop12)
pair_glmm_prop12 <- emmeans(glmm_prop12, ~ treatment_edge, type = "response")
pairs(pair_glmm_prop12)

# GLMM prop 2018
glmm_prop18 <- glmmTMB::glmmTMB(ecm_abun ~ treatment_edge +
                        (1 | plot),
                        family = beta_family(link = "logit"),
                        data = ecm_abun_df18) 
...
summary(glmm_prop18)
pair_glmm_prop18 <- emmeans(glmm_prop18, ~ treatment_edge, type = "response")
...
genus_nb_hala <- as.factor(ecm_phy@tax_table[,5]) %>% droplevels() %>% levels() %>% length() %>% print()
## OTU Genus count (0;1) by treatment
ecm_phy_ggplot1_prop <- phyloseq::plot_bar(as_binary_otu_table(ecm_phy), x = "treatment_edge") 
ecm_phy_ggplot1_prop <- ggplot(ecm_phy_ggplot1_prop$data, aes(fill= fct_reorder(as.factor(Genus), Abundance), x = factor(treatment_edge, levels = order_level), y = Abundance))+
...
# OTU Genus abundance by treatment
ecm_phy_ggplot2_prop <- phyloseq::plot_bar(ecm_phy, x = "treatment_edge") 
ecm_phy_ggplot2_prop <- ggplot(ecm_phy_ggplot2_prop$data, aes(fill= fct_reorder(as.factor(Genus), Abundance), x = factor(treatment_edge, levels = order_level), y = Abundance))+
  geom_bar(stat="identity", position = "fill")+
  scale_fill_manual(values = funky(genus_nb_hala))+
  facet_grid(~year, scale="free")+
...
ecm_phy_fruit1 <- phyloseq::plot_bar(ecm_phy, x = "treatment_edge") 
ecm_phy_fruit1 <- ggplot(ecm_phy_fruit1$data, aes(fill= fct_reorder(as.factor(Fruitbody), Abundance), x = treatment_edge, y = Abundance))+
  geom_bar(stat = "identity", position = "stack") +
...
## % 
fruit_count <- ecm_phy_fruit1$data %>% group_by(Fruitbody, year, treatment_edge) %>%
...
jaccard12 <- plot_ordination(ecm_12, jaccard12_ord, type="samples", color="treatment_edge",label="sample_id")+
  geom_point(size=2)+
  # color and elipse
  scale_color_manual(values = coltreatment_raref)+
  stat_ellipse(type="t", level=0.70)+
...
bray12 <- plot_ordination(ecm_12, bray12_ord, type="samples", color="treatment_edge",label="sample_id")+
  geom_point(size=2)+
  # color and elipse
  scale_color_manual(values = coltreatment_raref)+
  stat_ellipse(type="t", level=0.70)+
...
jaccard18 <- plot_ordination(ecm_18, jaccard18_ord, type="samples", color="treatment_edge",label="sample_id")+
  geom_point(size=2)+
  # color and elipse
  scale_color_manual(values = coltreatment_raref)+
  stat_ellipse(type="t", level=0.70)+
...
bray18 <- plot_ordination(ecm_18, bray18_ord, type="samples", color="treatment_edge",label="sample_id")+
  geom_point(size=2)+
  scale_color_manual(values = coltreatment_raref)+
  stat_ellipse(type="t", level=0.70)+
  guides(color=guide_legend(title="Distance from SC"))+
...
perm_12 # SI
pw_perm12 <-pairwise.adonis2(t(ecm_12@otu_table) ~ treatment_edge, data = data.frame(ecm_12@sam_data))
pw_perm12 
...
htmap12 <- plot_heatmap(yr_12, method = "MDS", distance = "bray",
             sample.label = "treatment_edge", sample.order = order_level,
             taxa.label = "OTU_ID", 
             low = "grey", high = "red", na.value = "white")
htmap12
...
```
```{r circle graph}
circle_pq(ecm_phy, "treatment_edge", taxa = "Genus", add_nb_seq = F,
          grid_col = "darkgrey",
...
## 3.1 stat test SY
```{r DOWNSTREAM EFFECT }
SC_stream <- filter(df_div, treatment_edge %in% "SC") %>% arrange(treatment_edge) 
for_stream <- filter(df_div, treatment_edge %in% "FC") %>% arrange(treatment_edge) 

bxplt_rich_str <- df_div_str %>%
  ggplot(aes(x = treatment_edge, y = rich_str, fill =  stream))+
  geom_boxplot(width = 0.4, color = "black")+
  facet_grid(~year)
bxplt_rich_str
 ...
 coord_cartesian(ylim=c(0,15))+
  ...
  facet_grid(~ year) +
  theme(legend.position="none", plot.title = element_text(size = 14)) +
  labs(title = "Richness in OTU by distance from cutting")+
  xlab("Position from cutting")+
  scale_fill_manual(values = coltreatment)+
...
par(mfrow = c(1, 2))
hist(rich, breaks = seq(0, 15, by = 1))
hist(div, breaks = seq(0, 3, by = 0.2))
par(mfrow = c(1, 1))
...
summary(rich)
hist(rich, breaks = seq(0, 15, by = 1))
glm_rich <- glm(rich ~ treatment_edge * year, 
                      family = poisson(link = "log"),
                      data = df_div)

hist(residuals.glm(glm_rich))
par(mfrow = c(2, 2))
plot(glm_rich) # graphical assumption tests
par(mfrow = c(1, 1))
...
hala_rrcurve_groups1 <- phyloseq.extended::ggrare(yr_12, step = 10, color = "treatment_edge",plot=F, se=T)+ 
  ...
  scale_color_manual(values = c("FE" = "orange",
                                "SC" = "darkred",
                                "FC" = "darkgreen"))+
  scale_fill_manual(values = c("FE" = "orange",
                                "SC" = "darkred",
                                "FC" = "darkgreen"))
...
hala_hill0 <- hala_hill_treatment[[1]]+ 
  labs(title="Hill 0 : position from SC", subtitle="Hagadalen_edge")+
  coord_cartesian(xlim=c(0,16))+
  scale_x_continuous(name="Hill_0", breaks=seq(0,16,by=2))+
  scale_y_continuous(name="Position from SC", breaks=c(1,2,3))+
  scale_color_manual(values = c("SC" = "darkred",
                                "FE" = "orange",
                                "FC" = "darkgreen"))+
  theme(axis.text.x=element_text(color="coral4", size=8, angle=0))
...
hala_hill1 <- hala_hill_treatment[[2]]+ 
  labs(title="Hill 1 : position from SC", subtitle="Hagadalen_edge")+
  coord_cartesian(xlim=c(0,12))+
  scale_x_continuous(name="Hill_1", breaks=seq(0,12,by=2))+
  scale_y_continuous(name="Position from SC", breaks=c(1,2,3))+
  scale_color_manual(values = c("FE" = "orange",
...
hala_hill2 <- hala_hill_treatment[[3]]+ 
  labs(title="Hill 2 : position from SC", subtitle="Hagadalen_edge")+
  coord_cartesian(xlim=c(0,10))+
  scale_x_continuous(name="Hill_2", breaks=seq(0,10,by=2))+
  scale_y_continuous(name="Position from SC", breaks=c(1,2,3))+
  scale_color_manual(values = c("FE" = "orange",
...
ecm_phy_ggplot2 <- ggplot(ecm_phy_ggplot2$data, aes(fill= fct_reorder(as.factor(Genus), Abundance), x = factor(treatment_edge, levels = order_level), y = Abundance))+
  ...
  scale_fill_manual(values = funky(genus_nb_hala))+
  facet_grid(~year, scale="free")+
  labs(title="OTU Genus abundance by distance from cut and by year", subtitle = "Halasen_edge")+
  xlab("Distance from cut")+
...
ecm_phy_ggplot3 <- ggplot(ecm_phy_ggplot3$data, aes(fill= fct_reorder(as.factor(OTU_ID), Abundance), x=treatment_edge, y = Abundance))+
  ...
  scale_fill_manual(values = funky(otu_nb_hala))+
  facet_grid(~year, scale="free")+
  labs(title="OTU Genus count (0;1) by position from SC and year", subtitle = "Halasen_edge")+
  xlab("position from SC")+
  guides(fill=guide_legend(title="Genus"))+
  theme(axis.text.x=element_text(color="coral4", size=8, angle=20))
ecm_phy_ggplot3
...
## 3.2.2 multivariate analysis SY
```{r multivariate representation}
ecm_phy_ordi_mdsjaSC <- ordinate(as_binary_otu_table(ecm_phy), method = "MDS", dist="jaSCard")
ecm_phy_ordi_mdsjaSC <- plot_ordination(ecm_phy, ecm_phy_ordi_mdsjaSC, type="samples", color="treatment_edge",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
  scale_color_manual(values = c("FE" = "orange",
