...
Hellinger transformation is sqrt (reads of species / total reads in sample) ie sqrt (relative abudances)
 ...
 - Zero oSCurence of OTUs or no reads in some samples -> leads to a biais:
...
## PER SAMPLE TYPE :
# 2012
n_otu_ecm_12 <- nrow(ecm_12@tax_table) %>% print() # ECM
nb_rd_ecm_12 <- sum(ecm_12@otu_table) %>% print() # ECM
# 2018
n_otu_ecm_18 <- nrow(ecm_18@tax_table) %>% print() # ECM
nb_rd_ecm_18 <- sum(ecm_18@otu_table) %>% print() # ECM

### FCEST:
n_otu_ecm_for <- nrow(ecm_for@tax_table) %>% print() # ECM
nb_rd_ecm_for <- sum(ecm_for@otu_table) %>% print() # ECM

### FEE
n_otu_ecm_edg <- nrow(ecm_edg@tax_table) %>% print() # ECM
nb_rd_ecm_edg <- sum(ecm_edg@otu_table) %>% print() # ECM

### SC
n_otu_ecm_cut <- nrow(ecm_cut@tax_table) %>% print() # ECM
nb_rd_ecm_cut <- sum(ecm_cut@otu_table) %>% print() # ECM

#### POURC:
pourc_12 <- (nb_rd_ecm_12 / (nb_rd_ecm_12 + nb_rd_ecm_18)*100) %>% print()
pourc_18 <- (nb_rd_ecm_18 / (nb_rd_ecm_12 + nb_rd_ecm_18)*100) %>% print()

pourc_for <- (nb_rd_ecm_for / (sum(ecm_phy@otu_table))*100) %>% print()
pourc_edg <- (nb_rd_ecm_edg / (sum(ecm_phy@otu_table))*100) %>% print()
pourc_cut <- (nb_rd_ecm_cut / (sum(ecm_phy@otu_table))*100) %>% print()
```
...
# hala
miss_sam_clust_hala <- anti_join(x = sam_hala[,1:7], y = ecm_phy@sam_data[,1:7]) # exclude treatment_code column because we changed factor values for this column
...
### NEW 
circle_pq(physeq, "site", taxa = "Genus", add_nb_seq = F) # only 53,57% plotted


bxplt_rich <- df_div %>%
  ggplot(aes(x = factor(treatment_code), y = rich))+
...
bxplt_relat <- ecm_abun_df %>% ggplot(aes(x = factor(treatment_code), y = ecm_abun))+
  geom_boxplot(width = 0.4, color = "black") +
...
# post biomol design
table_postbiomol <- table(sam_hala$treatment_code, sam_hala$plot) # post biomol
...
### plots: 2 (FC, n=1); 9 (FC, n=2)
n_postbiomol <- length(sam_hala$sample_id) %>% print()
# balanced now after resampling of forest 

# post bioinf design
table_postbioinf <- table(ecm_phy@sam_data$treatment_code, ecm_phy@sam_data$plot) # post bioinf
...
summary(as.factor(ecm_phy@sam_data$treatment_code))

tableall <- table(ecm_phy@sam_data$treatment_code, ecm_phy@sam_data$plot) %>% print() %>% as.data.frame.matrix() 
...
bxplt_rich <- df_div %>%
  ggplot(aes(x = factor(treatment_code, levels = order_level), y = rich, fill =  treatment_code))+
  geom_boxplot(width = 0.4, color = "black") +
  coord_cartesian(ylim=c(0,15))+
  scale_y_continuous(name = "OTU richness", breaks = seq(0, 15, by = 2))+
  theme(axis.text.x=element_text(color="coral4", size = 11, angle = 0))+
...
# variation 
mean12 <- aggregate(df_div12$rich, by = list(df_div12$treatment_code), FUN = mean) %>% print()

sd12 <- aggregate(df_div12$rich, by = list(df_div12$treatment_code), FUN = sd) %>% print()

mean18 <- aggregate(df_div18$rich, by = list(df_div18$treatment_code), FUN = mean) %>% print()
sd18 <-  aggregate(df_div18$rich, by = list(df_div18$treatment_code), FUN = sd) %>% print()


summary(as.factor(df_div$treatment_code))
```
...
phy_ggplot1 <- phyloseq::plot_bar(as_binary_otu_table(phy), x = "treatment_code") 

phy_ggplot1 <- ggplot(phy_ggplot1$data, aes(fill= fct_reorder(as.factor(primary_lifestyle), Abundance), x = factor(treatment_code, levels = order_level), y = Abundance))+
...
# plotting 
bxplt_comp <- df_div_relat %>% ggplot(aes(x = factor(treatment_code, levels = order_level), y = richness, fill =  primary_lifestyle))+
  geom_boxplot(width = 0.35, color = "black") +
  coord_cartesian(ylim=c(0,15))+
  scale_y_continuous(name = "OTU richness", breaks = seq(0, 15, by = 2))+
  theme(axis.text.x=element_text(color="coral4", size = 11, angle = 0))+
  facet_grid(~ year) +
  theme(legend.position="right", plot.title = element_text(size = 14)) +
  labs(title = "OTU richness by cutting position and lifestyle")+
  xlab("Position from cutting")+
  scale_fill_manual(values = c("#626567", "#D7DBDD"))
bxplt_comp

bxplt_relat <- ecm_abun_df %>% ggplot(aes(x = factor(treatment_code, levels = order_level), y = ecm_abun, fill = treatment_code))+
  geom_boxplot(width = 0.4, color = "black") +
  coord_cartesian(ylim=c(0,1))+
  scale_y_continuous(name = "ECM proportion", breaks = seq(0, 1, by = 0.1))+
  theme(axis.text.x=element_text(color="coral4", size = 11, angle = 0))+
...
# variation 
sd12_prop <- aggregate(ecm_abun_df12$ecm_abun, by = list(ecm_abun_df12$treatment_code), FUN = sd) %>% print()
sd18_prop <-  aggregate(ecm_abun_df18$ecm_abun, by = list(ecm_abun_df18$treatment_code), FUN = sd) %>% print()
mean(ecm_abun_df18$ecm_abun[ecm_abun_df18$treatment_code == "FE"])

mean12_prop <- aggregate(ecm_abun_df12$ecm_abun, by = list(df_div12$treatment_code), FUN = mean) %>% print()
mean18_prop <- aggregate(ecm_abun_df18$ecm_abun, by = list(df_div18$treatment_code), FUN = mean) %>% print()
```
```{r MODEL }
# https://stats.stackexchange.com/questions/31569/questions-about-how-random-effects-are-specified-in-lmer
## for constructing model with random effects
...
# not possible to put in random effect a variable that is NA for levels of other variable (eg stream with FE from treatment)
...
# GLMM richness 
glmm_rich <- lme4::glmer(rich ~ treatment_code * year +
                           (1 | plot),
...
summary(glmm_rich)
pair_glmm_rich <- emmeans(glmm_rich, ~ treatment_code * year, type = "response")
pairs(pair_glmm_rich)
tab_model(glmm_rich, show.df = TRUE)

# GLMM richness 2012
glmm_rich12 <- lme4::glmer(rich ~ treatment_code +
                           (1 | plot),
                         family = poisson(link = "log"),
                         data = df_div12) 
...
lrtest(glmm_rich12)
pair_glmm_rich12 <- emmeans(glmm_rich12, ~ treatment_code, type = "response")
pairs(pair_glmm_rich12)

# GLMM richness 2018
glmm_rich18 <- lme4::glmer(rich ~ treatment_code +
                           (1 | plot),
                         family = poisson(link = "log"),
                         data = df_div18) 
...
summary(glmm_rich18)
lrtest(glmm_prop18)
pair_glmm_rich18 <- emmeans(glmm_rich18, ~ treatment_code, type = "response")
pairs(pair_glmm_rich18)

# GLMM prop
glmm_prop <- glmmTMB::glmmTMB(ecm_abun ~ treatment_code * year +
                        (1 | plot),
                        family = beta_family(link = "logit"),
                        data = ecm_abun_df) 
car::Anova(glmm_prop, type = "III")
summary(glmm_prop)
pair_glmm_prop <- emmeans(glmm_prop, ~ treatment_code * year, type = "response")
pairs(pair_glmm_prop)

# GLMM prop 2012
glmm_prop12 <- glmmTMB::glmmTMB(ecm_abun ~ treatment_code +
                        (1 | plot),
                        family = beta_family(link = "logit"),
                        data = ecm_abun_df12)
...
summary(glmm_prop12)
pair_glmm_prop12 <- emmeans(glmm_prop12, ~ treatment_code, type = "response")
pairs(pair_glmm_prop12)

# GLMM prop 2018
glmm_prop18 <- glmmTMB::glmmTMB(ecm_abun ~ treatment_code +
                        (1 | plot),
                        family = beta_family(link = "logit"),
                        data = ecm_abun_df18) 
...
summary(glmm_prop18)
pair_glmm_prop18 <- emmeans(glmm_prop18, ~ treatment_code, type = "response")
...
## intercept = FE + 2012 + FE:2012, necessarily significant because model no null (0)
## SC and FC are S=/= of FE independently of year     NB: (S=/= means significantly different)
## year 2018 is =/= from 2012 independently of treatment
## SC:2018 (ie in interaction) is =/= from FE:2012 -> SC and FE difference do vary through years with an observed "inversment" where richness of the edges decrease through time when richness of middle is increasing.
## FC:2018 is =/= from FE:2012 -> FC and FE difference do not vary through years (a global decrease for both of treatments is observed)

# summary interpretation of correlations:

anova(glmm, test = "LR") # order influence significance intercept (FE select as intercept, alphabetically or 1st row of df.)
...
tuk <- emmeans(glm, "treatment_code")
contrast(tuk, "pairwise", adjust = "Tukey")

emmeans(glm, pairwise ~ treatment_code : year) # possible to do pairwise on interaction
emmeans(lm, pairwise ~ treatment_code)
TukeyHSD(aov(lm))
...
boxplot(rich ~ treatment_code, data = df_div)
...
hala_rrcurve_groups1 <- phyloseq.extended::ggrare(yr_12, step = 10, color = "treatment_code",plot=F, se=T)+ 
  facet_wrap(~ year)+
...
hala_rrcurve_groups2 <- phyloseq.extended::ggrare(yr_18, step = 10, color = "treatment_code",plot=F, se=T)+ 
  facet_wrap(~year)+
  geom_vline(xintercept = min(rowSums(yr_18@otu_table)), color = "coral4")+
...
hala_rrcurve <- phyloseq.extended::ggrare(ecm_phy, step = 10, color = "treatment_code", plot = F, se=T)+
  labs(x = "Group size",
       y =  "OTU richness",
...
plot_richness(ecm_phy, measures = c("Chao1", "ACE", "InvSimpson"), nrow=3, sortby = "Chao1", x = "treatment_code")

hala_hill_treatment <- hill_pq(physeq = ecm_phy_t, 
                               variable = "treatment_code",
                               color_fac = NA, 
                               letters = F)
...
## OTU Genus count (0;1) by treatment
ecm_phy_ggplot1 <- phyloseq::plot_bar(as_binary_otu_table(ecm_phy), x = "treatment_code") 
ecm_phy_ggplot1 <- ggplot(ecm_phy_ggplot1$data, aes(fill= fct_reorder(as.factor(Genus), Abundance), x = factor(treatment_code, levels = order_level), y = Abundance))+
  geom_bar(stat = "identity", position = "stack") +
...
ecm_phy_ggplot2 <- phyloseq::plot_bar(ecm_phy, x = "treatment_code") 
ecm_phy_ggplot2 <- ggplot(ecm_phy_ggplot2$data, aes(fill= fct_reorder(as.factor(Genus), Abundance), x = factor(treatment_code, levels = order_level), y = Abundance))+
  geom_bar(stat="identity", position = "stack")+
  scale_fill_manual(values = funky(genus_nb_hala))+
  facet_grid(~year, scale="free")+
...
genus_nb_hala <- as.factor(ecm_phy@tax_table[,5]) %>% droplevels() %>% levels() %>% length() %>% print()
## OTU Genus count (0;1) by treatment
ecm_phy_ggplot1_prop <- phyloseq::plot_bar(as_binary_otu_table(ecm_phy), x = "treatment_code") 
ecm_phy_ggplot1_prop <- ggplot(ecm_phy_ggplot1_prop$data, aes(fill= fct_reorder(as.factor(Genus), Abundance), x = factor(treatment_code, levels = order_level), y = Abundance))+
...
# OTU Genus abundance by treatment
ecm_phy_ggplot2_prop <- phyloseq::plot_bar(ecm_phy, x = "treatment_code") 
ecm_phy_ggplot2_prop <- ggplot(ecm_phy_ggplot2_prop$data, aes(fill= fct_reorder(as.factor(Genus), Abundance), x = factor(treatment_code, levels = order_level), y = Abundance))+
  geom_bar(stat="identity", position = "fill")+
  scale_fill_manual(values = funky(genus_nb_hala))+
  facet_grid(~year, scale="free")+
...
ecm_phy_ggplot3 <- phyloseq::plot_bar(as_binary_otu_table(ecm_phy), x = "treatment_code") 
ecm_phy_ggplot3 <- ggplot(ecm_phy_ggplot3$data, aes(fill= fct_reorder(as.factor(OTU_ID), Abundance), x=treatment_code, y = Abundance))+
  geom_bar(stat="identity", position = "stack") +
...
ggplot(data = ., aes(x = treatment_code, y = Abundance)) +
...
hala_box2 <- boxplot_abundance(d= ecm_phy, x='treatment_code', y='otu_table',
   line='subject')
```
...
ecm_phy_fruit1 <- phyloseq::plot_bar(ecm_phy, x = "treatment_code") 
ecm_phy_fruit1 <- ggplot(ecm_phy_fruit1$data, aes(fill= fct_reorder(as.factor(Fruitbody), Abundance), x = treatment_code, y = Abundance))+
  geom_bar(stat = "identity", position = "stack") +
...
## % 
fruit_count <- ecm_phy_fruit1$data %>% group_by(Fruitbody, year, treatment_code) %>%
...
pourc_asc <- (asc / (asc + bas))*100
pourc_bas <- (bas / (asc + bas))*100
...
ecm_phy_ordi_mdsjacc <- plot_ordination(ecm_phy, ecm_phy_ordi_mdsjacc, type = "samples", color="treatment_code",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
  scale_color_manual(values = coltreatment_raref)+
...
ecm_phy_ordi_mdsbray <- plot_ordination(ecm_phy, ecm_phy_ordi_mdsbray, type="samples", color="treatment_code",label="sample_id")+
  geom_point(size=2)+
  # color and elipse
  scale_color_manual(values = coltreatment_raref)+
  stat_ellipse(type="t", level=0.70)+
...
jaccard12 <- plot_ordination(ecm_12, jaccard12_ord, type="samples", color="treatment_code",label="sample_id")+
  geom_point(size=2)+
  # color and elipse
  scale_color_manual(values = coltreatment_raref)+
  stat_ellipse(type="t", level=0.70)+
...
bray12 <- plot_ordination(ecm_12, bray12_ord, type="samples", color="treatment_code",label="sample_id")+
  geom_point(size=2)+
  # color and elipse
  scale_color_manual(values = coltreatment_raref)+
  stat_ellipse(type="t", level=0.70)+
...
jaccard18 <- plot_ordination(ecm_18, jaccard18_ord, type="samples", color="treatment_code",label="sample_id")+
  geom_point(size=2)+
  # color and elipse
  scale_color_manual(values = coltreatment_raref)+
  stat_ellipse(type="t", level=0.70)+
...
bray18 <- plot_ordination(ecm_18, bray18_ord, type="samples", color="treatment_code",label="sample_id")+
  geom_point(size=2)+
  scale_color_manual(values = coltreatment_raref)+
  stat_ellipse(type="t", level=0.70)+
  guides(color=guide_legend(title="Distance from SC"))+
...
hala_ordi_specvalues_mdsjaSC <- ordinate(as_binary_otu_table(ecm_phy), method = "MDS", dist="jaSCard")
ecm_phy_ordi_mdsjaSC <- plot_ordination(ecm_phy, hala_ordi_specvalues_mdsjaSC, type="samples", color="specification_value",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
  ...
  # legend
  guides(color=guide_legend(title="position from SC"))+
  ggtitle(label = "MDS of samples from Halasen_edge by year", subtitle="presence-absence (jaSCard distance)")
ecm_phy_ordi_mdsjaSC
```
```{r multivariate models}
perm <- adonis_pq(ecm_phy, "treatment_code * year")
perm

perm_treatment <- adonis_pq(ecm_phy, "treatment_code")
perm_treatment
...
# separating years 
## best imo 
perm_12 <- adonis_pq(ecm_12, "treatment_code")
perm_12 # SI
pw_perm12 <-pairwise.adonis2(t(ecm_12@otu_table) ~ treatment_code, data = data.frame(ecm_12@sam_data))
pw_perm12 

perm_18 <- adonis_pq(ecm_18, "treatment_code")
perm_18 # NS 
...
treat <- ifelse(grepl("FE", ht_count$yr_treat), "FE",
                         ifelse(grepl("SC", ht_count$yr_treat), "SC", "FC")) # nested ifelse
...
htmap12 <- plot_heatmap(yr_12, method = "MDS", distance = "bray",
             sample.label = "treatment_code", sample.order = order_level,
             taxa.label = "OTU_ID", 
             low = "grey", high = "red", na.value = "white")
htmap12
...
```
```{r circle graph}
circle_pq(ecm_phy, "treatment_code", taxa = "Genus", add_nb_seq = F,
          grid_col = "darkgrey",
...
```{r ordination}
hala5_ecm_ordi_mdsjaSC  <- ordinate(hala5_ecm, method = "MDS", dist="bray")
hala5_ecm_ordi_mdsjaSC  <- plot_ordination(hala5_ecm, hala5_ecm_ordi_mdsjaSC, type="samples", color="treatment",label="sample_id")+
  geom_point(size=2)+
  # color and elipse
...
hala5_ecm_ordi_mdsjaSC

hala5_ecm_ordi_mdsbray  <- ordinate(as_binary_otu_table(hala5_ecm), method = "MDS", dist="jaSCard")
...
hala5_ecm_ordi1 <- ordinate(as_binary_otu_table(hala5_ecm), method = "NMDS", dist="jaSCard",k=20)
...
```{r ordination}
fanga_ecm_ordi_mdsjaSC <- ordinate(as_binary_otu_table(fanga_ecm), method = "MDS", dist="jaSCard")
fanga_ecm_ordi_mdsjaSC <- plot_ordination(fanga_ecm, fanga_ecm_ordi_mdsjaSC, type="samples", color="treatment",label="sample_id")+
  geom_point(size=2)+
  facet_wrap(~year)+
  # color and elipse
  scale_color_manual(values = c("clearcut" = "darkred",
  ...
  # legend
  guides(color=guide_legend(title="position from SC"))+
  ggtitle(label = "MDS of samples from fangamon by year", subtitle="presence-absence (jaSCard distance)")
fanga_ecm_ordi_mdsjaSC
...
# every control and NA suppressed (ie 0 oSCurence) 
## 8 PCR neg => good control,
...
sac_f_raw # cluster size aSCumulation curve from all fungi clusters
```
```{r hala edge}
# design
table(hala_sam$plot,hala_sam$treatment)
...
ecm_phy_ordi_mdsjaSC # MDS jaSCard (0;1)
...
# hill numbers
fanga_hill0  # OTU richness # SC 2018 huge OTU richness for a single sample (can be seen also in distribution plots)
...
fanga_ecm_ordi_mdsjaSC # MDS jaSCard (0;1)
...
hala5_ecm_ordi_mdsjaSC # MDS jaSCard (0;1)
...
# alpha diversity
## rarecurves
ggsave(plot = hala_rrcurve_groups1, filename = file.path("rrcurve12","rrcurve12.png"))
ggsave(plot = hala_rrcurve_groups2, filename = file.path("rrcurve18","rrcurve18.png"))

## boxplot richness + proportion
ggsave(plot = bxplt_rich, filename = file.path("boxplot_rich","boxplot_rich.png"))
ggsave(plot = bxplt_relat, filename = file.path("boxplot_ecm_prop","boxplot_ecm_prop.png"))

# beta diversity
## multivariate analysis
ggsave(plot = ecm_phy_ordi_mdsjacc, filename = file.path("MDS_jacc","MDS_jacc.png"))
ggsave(plot = ecm_phy_ordi_mdsbray, filename = file.path("MDS_bc","MDS_bc.png"))

## genus distribution 
ggsave(plot = ecm_phy_ggplot1, filename = file.path("barplot_genus_01","barplot_genus_01.png"))
ggsave(plot = ecm_phy_ggplot2, filename = file.path("barplot_genus_reads","barplot_genus_reads.png"))

ggsave(plot = ecm_phy_ggplot1_prop, filename = file.path("barplot_genus_01_prop","barplot_genus_01_prop.png"))
ggsave(plot = ecm_phy_ggplot2_prop, filename = file.path("barplot_genus_reads_prop","barplot_genus_reads_prop.png"))
## heatmap  
ggsave(plot = htmap_all, filename = file.path("heatmap","heatmap.png"))

## occurence and composition
ggsave(plot = occurence_pourc_12, filename = file.path("occurence_pourc_12","occurence_pourc_12.png"))
ggsave(plot = occurence_pourc_18, filename = file.path("occurence_pourc_18","occurence_pourc_18.png"))
