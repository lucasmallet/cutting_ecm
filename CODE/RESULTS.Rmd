...
table18_missing["CC", "3"] <- 2 # check manual 
...
bxplt_shan <- df_div %>%
  ggplot(aes(x = factor(treatment_edge, levels = order_level), y = div, fill =  treatment_edge))+
  geom_violin(width = 1, color = "black", alpha = 0.7) +
  geom_boxplot(width = 0.2, color = "black") +
  coord_cartesian(ylim=c(0, 2.5))+
  scale_y_continuous(name = "OTU number", breaks=seq(0, 2.5, by = 0.2))+
  theme(axis.text.x=element_text(color="coral4", size = 11, angle = 0))+
  facet_grid(~ year) +
  theme(legend.position="none", plot.title = element_text(size = 14)) +
  labs(title = "Richness in OTU by cutting position")+
...
bxplt_even <- df_div %>%
  ggplot(aes(x = factor(treatment_edge, levels = order_level), y = even, fill =  treatment_edge))+
  geom_violin(width = 1, color = "black", alpha = 0.7) +
  geom_boxplot(width = 0.2, color = "black") +
  coord_cartesian(ylim=c(0, 1))+
  scale_y_continuous(name = "OTU number", breaks=seq(0, 1, by = 0.1))+
  theme(axis.text.x=element_text(color="coral4", size = 11, angle = 0))+
  facet_grid(~ year) +
  theme(legend.position="none", plot.title = element_text(size = 14)) +
  labs(title = "Richness in OTU by cutting position")+
  ...
  scale_fill_manual(values = coltreatment)+
  stat_summary(fun.data = give.n, geom = "text", vjust = 2)
bxplt_even # N - 12 because of NaN values (samplings with only 1 OTU)
...
glmm_shan <- lme4::lmer(log(div) ~ treatment_edge * year +
                           (1 | plot),
                         data = df_div) 
...
## CC and FOR are S=/= of EDG independently of year     NB: (S=/= means significantly different)
## year 2018 is =/= from 2012 independently of treatment
## CC:2018 (ie in interaction) is =/= from EDG:2012 -> CC and EDG difference do vary through years with an observed "inversment" where richness of the edges decrease through time when richness of middle is increasing.
...
hala_rrcurve_groups1 <- phyloseq.extended::ggrare(yr_12, step = 10, color = "treatment_edge",plot=F, se=T)+ 
  ...
  labs(x = "Group size (ie sum of samples reads per group)",
       y =  "OTU richness",
       title = "Rarefaction curves by treatment group",
       color = "Treatment")+
  guides(fill = F)+ # Suppressing the filling (standard error) legend 
...
hala_rrcurve_groups2 <- phyloseq.extended::ggrare(yr_18, step = 10, color = "treatment_edge",plot=F, se=T)+ 
  ...
  guides(fill = F)+ # Suppressing the filling (standard error) legend 
  theme(legend.position = c(0.85, 0.6))+
  annotate("text", x = 220, y = 3, label = "min treatment group size (n = 205)", size = 3.5, angle=0, color = "coral4", hjust = 0)+
  scale_color_manual(values = coltreatment_raref)+
  scale_fill_manual(values = coltreatment_raref)
...
hala_hill0 <- hala_hill_treatment[[1]]+ 
  ...
  scale_color_manual(values = c("CC" = "darkred",
                                "EDG" = "orange",
                                "FOR" = "darkgreen"))+
...
treat <- ifelse(grepl("EDG", ht_count$yr_treat), "EDG",
                         ifelse(grepl("CC", ht_count$yr_treat), "CC", "FOR")) # nested ifelse
...
htmap_all <- ggplot(ht_count,  aes(x = factor(yr_treat, levels = order_level_mg), y = Genus, fill = countfactor)) + 
  ...
  scale_fill_manual(values = rev(c("grey90", RColorBrewer::brewer.pal(n = 6, name = "YlOrRd"))), na.value = "grey90")+
  theme_grey(base_size = 10)+
  guides(fill =guide_legend(title = str_wrap("Number of sequence reads (rarefied)", width = 20)))+
  labs(x = "Position to CC and year", y = "Genus", title = "Occurence of OTU Genus by position to CC and year")+
...
## 3.1 stat test SY
```{r DOWNSTREAM EFFECT }
cc_stream <- filter(df_div, treatment_edge %in% "CC") %>% arrange(treatment_edge) 
