...
```
```{r objects}
coltreatment <- c("EDG" = "khaki2",
                  "CUT" = "coral2",
                  "FOR" = "springgreen4") # palette for treatment

coltreatment_raref <- c("EDG" = "orange",
                                "CUT" = "red3",
                                "FOR" = "springgreen4")

order_level <- c("FOR", "EDG", "CUT") # ordination of factors (aes = factor(x, levels = order_level))
order_level_mg <- c("2012FOR", "2012EDG", "2012CUT", "2018FOR", "2018EDG", "2018CUT")
order_level_simpl <- c("FOR_12", "EDG_12", "CUT_12", "FOR_18", "EDG_18", "CUT_18")
```
...
raw_clust$Cluster_ID <- gsub('scata5670_','c',raw_clust$Cluster_ID) # replace character oCUTurence in all cells
...
sam_hala <- within(sam_hala, treatment_edge[treatment_edge=="middle" & treatment=="forest"] <- "FOR") 
...
for_subset12 <- sam_hala12[sam_hala12$treatment_edge == "FOR", ]
...
equal_sample12 <- rbind(
  sam_hala12[sam_hala12$treatment_edge == "CUT", ],
  sam_hala12[sam_hala12$treatment_edge == "EDG", ],
  for_subset12
)
...
## FOR 2018
# Subsample 8 observations from the "FOR" category
for_subset18 <- sam_hala18[sam_hala18$treatment_edge == "FOR", ]
for_subset18 <- for_subset18[sample(nrow(for_subset18), 8), ]

# Combine the subsampled "FOR" category with the other categories
equal_sample18 <- rbind(
  sam_hala18[sam_hala18$treatment_edge == "CUT", ],
  sam_hala18[sam_hala18$treatment_edge == "EDG", ],
  for_subset18
)
