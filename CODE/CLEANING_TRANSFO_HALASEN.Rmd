...
rownames(fruit) <- fruit$X
fruit <- fruit[, -1]
colnames(fruit) <- gsub("X", "", colnames(fruit))
...
raw_clust$Cluster_ID <- gsub('scata5670_','c',raw_clust$Cluster_ID) # replace character oSCurence in all cells
...
factor(sam_hala$specification) %>% levels()
sam_hala$treatment_edge <- plyr::revalue(sam_hala$specification,
                           c("edge_north"="edge", 
                             "edge_south"="edge",
                             "kant_norra"="edge", 
                             "middle_lower"="middle", 
                             "middle_upper"="middle",
                             "midle"="middle"))
as.factor(sam_hala$treatment_edge) %>% droplevels() %>% levels()

sam_hala$specification_value <- plyr::revalue(sam_hala$specification,
                           c("edge_north"="N", 
                             "edge_south"="S",
                             "kant_norra"="N", 
                             "middle_lower"="LO", 
...
sam_hala <- within(sam_hala, treatment_edge[treatment_edge=="middle" & treatment=="forest"] <- "FC") 

sam_hala$treatment_edge <- plyr::revalue(sam_hala$treatment_edge,
                           c("middle"="SC", 
                             "edge"="FE"))
as.factor(sam_hala$treatment_edge) %>% droplevels() %>% levels()

sam_hala12 <- subset(sam_hala, year == "2012")
sam_hala18 <- subset(sam_hala, year == "2018")
```
```{r random resampling -> balanced design}
table(sam_hala12$treatment_edge)
table(sam_hala18$treatment_edge)
table(sam_hala$treatment_edge)
sam_hala$treatment_edge <- as.factor(sam_hala$treatment_edge)
...
for_subset12 <- sam_hala12[sam_hala12$treatment_edge == "FC", ]
...
equal_sample12 <- rbind(
  sam_hala12[sam_hala12$treatment_edge == "SC", ],
  sam_hala12[sam_hala12$treatment_edge == "FE", ],
  for_subset12
)

# Generate the table of sample sizes for each category
table(equal_sample12$treatment_edge)
sam_hala12 <- equal_sample12


## FC 2018
# Subsample 8 observations from the "FC" category
for_subset18 <- sam_hala18[sam_hala18$treatment_edge == "FC", ]
for_subset18 <- for_subset18[sample(nrow(for_subset18), 8), ]

# Combine the subsampled "FC" category with the other categories
equal_sample18 <- rbind(
  sam_hala18[sam_hala18$treatment_edge == "SC", ],
  sam_hala18[sam_hala18$treatment_edge == "FE", ],
  for_subset18
)

# Generate the table of sample sizes for each category
table(equal_sample18$treatment_edge)
sam_hala18 <- equal_sample18

sam_hala <- rbind(sam_hala12, sam_hala18)
```
...
otu_not_in_hala <- which(colSums(otu_fungi_t) == 0) # 1182 clusters
otu_fungi_t <- otu_fungi_t[, -otu_not_in_hala]
...
nrow(otu_001_transf)
nrow(otu_001)
nrow(otu_001_transf) - nrow(otu_001) # 1930 clusters with relative abundance < 1% = suppressed -> 299 conserved 

otu_notrarefied <- otu_001

# data properties BEFCE - AFTER suppresion of <1%
...
```{r otu rarefaction}
# data properties BEFCE rarefaction
rd_sps <- data.frame(rownames(otu_001), rowSums(otu_001)) # read sum per sample
hist(rd_sps$rowSums.otu_001.)
...
physeq@sam_data <- within(physeq@sam_data, treatment_edge[treatment_edge=="middle" & treatment=="forest"] <- "FC") 
...
# by treatment
ecm_for <- subset_samples(ecm_phy, ecm_phy@sam_data$treatment_edge == "FC") %>% clean_pq(remove_empty_samples = T,remove_empty_taxa = T, clean_samples_names = T)

ecm_edg <- subset_samples(ecm_phy, ecm_phy@sam_data$treatment_edge == "FE") %>% clean_pq(remove_empty_samples = T,remove_empty_taxa = T, clean_samples_names = T)

ecm_cut <- subset_samples(ecm_phy, ecm_phy@sam_data$treatment_edge == "SC") %>% clean_pq(remove_empty_samples = T,remove_empty_taxa = T, clean_samples_names = T)
...
phy_relat <- tax_glom(phy, taxrank = "primary_lifestyle")
...
ecm_abun_df <- data.frame(phy_relat2@sam_data$sample_id, phy_relat2@sam_data$plot, phy_relat2@sam_data$year, phy_relat2@sam_data$treatment_edge, ecm_abun)
colnames(ecm_abun_df) <- c("sample_id", "plot", "year", "treatment_edge", "ecm_abun")
ecm_abun_df[, c(2:4)] <- lapply(ecm_abun_df[, c(2:4)], as.factor) # tranforming columns into factors
...
osam_count <- osam[, c(5, 9 ,14:length(colnames(osam)))] %>% 
  group_by(treatment_edge, year) %>% 
  summarise_all(sum)

# order df 
osam_count <- osam_count[
  with(osam_count, order(year, rev(treatment_edge))),
] %>% as.data.frame()
...
Nt12_df <- data.frame(treatment_edge = c("SC", "FE", "FC"),year = "2012",  N = Ntreat12)
Nt18_df <- data.frame(treatment_edge = c("SC", "FE", "FC"), year = "2018", N = Ntreat18)
N_df <- rbind(Nt12_df, Nt18_df)
N_df <- N_df[
  with(N_df, order(year, rev(treatment_edge))),
...
osam12 <- subset(osam_count_df, year == "2012")
osam18 <- subset(osam_count_df, year == "2018")
...
# Apply the calc_prop() function to all columns and get a list of proportion vectors
...
var11 <- as.character(get_variable(pourc_genusdf, "year"))
var22 <- as.character(get_variable(pourc_genusdf, "treatment_edge"))
yr_treat_genus <- mapply(paste0, var11, var22, collapse = "_")
colnames(osam_count_df)
rownames(pourc_genusdf) <- yr_treat_genus
pourc_genusdf1 <- cbind(yr_treat_genus, pourc_genusdf) %>% round_df(1) # rounding
pourc_genusdf12 <- pourc_genusdf1[c(1:3), ] # subsets
pourc_genusdf18 <- pourc_genusdf1[c(4:6), ]


# remove 0 samples:
pourc_genusdf12_otu <- subset(pourc_genusdf12[c(2:ncol(pourc_genusdf12))], select = -which(colSums(pourc_genusdf12[c(2:ncol(pourc_genusdf12))]) == 0))

pourc_genusdf12 <- cbind(yr_treat_genus = yr_treat_genus[c(1:3)], pourc_genusdf12_otu)

pourc_genusdf18_otu <- subset(pourc_genusdf18[c(2:ncol(pourc_genusdf18))], select = -which(colSums(pourc_genusdf18[c(2:ncol(pourc_genusdf18))]) == 0))

pourc_genusdf18 <- cbind(yr_treat_genus = yr_treat_genus[c(4:6)], pourc_genusdf18_otu)

pourc_genusdf12$yr_treat_genus <- gsub("2012", "", pourc_genusdf12$yr_treat_genus) # remove now 2012
pourc_genusdf18$yr_treat_genus <- gsub("2018", "", pourc_genusdf18$yr_treat_genus) # remove now 2018
###
pourc_genusdf12b <- pourc_genusdf12 %>%
  # convert data to long format
  gather(key = "Genus", value = "pourc", -yr_treat_genus) %>%
  # rename columns
  setNames(c("yr_treat_genus", "Genus", "pourc")) %>%
  # convert yr_treat_genus to factor
  mutate(yr_treat_genus = factor(yr_treat_genus)) %>%
  # convert pourc to numeric (also converts '-' to NA, gives a warning)
  mutate(pourc = as.numeric(pourc))

pourc_genusdf18b <- pourc_genusdf18 %>%
  # convert data to long format
  gather(key = "Genus", value = "pourc", -yr_treat_genus) %>%
  # rename columns
  setNames(c("yr_treat_genus", "Genus", "pourc")) %>%
