...
```{r taxtable manipulation }
taxeco_0 <- subset(taxeco_id[, c(3:length(taxeco_id))]) # supressing cluster_id, cluster_size 
taxeco2 <- subset(taxeco_id[, c(3:length(taxeco_id))]) 
taxeco2 <- matrix(as.character(unlist(taxeco2)),
              ncol = ncol(taxeco2),
              nrow = nrow(taxeco2)) # creating matrix 

# creating df of MANUAL ID fungi =/= from AUTO ID
## we are going to use the fact that taxeco df. is only composed of manual verification to discriminate OTUs

id_taxeco <- raw_f_clust %>%
  filter(Cluster_ID %in% taxeco_id$Cluster_ID) %>% # keeping cluster_ID form "raw_f_clust" found in "taxeco_id" (verified and manually IDed OTUs) 
  arrange(as.numeric(order)) # ordering by name (from "raw_cluster")
id_taxeco <- cbind("MANUAL_ID", id_taxeco)

unid_taxeco <- raw_f_clust %>%
  filter(!Cluster_ID %in% taxeco_id$Cluster_ID) %>%
  arrange(as.numeric(order)) # same as previously but with unverified / unidentified fungi

# re-merging after attribution of the ID_TYPE 
...
rownames(osam_count) <- str_c(osam_count$treatment_edge, osam_count$year, sep = "_")
