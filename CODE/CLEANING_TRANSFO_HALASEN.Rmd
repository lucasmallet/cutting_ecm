...
taxeco1 <- subset(taxeco_id[,c(3:length(taxeco_id)),]) 
taxeco1 <- matrix(as.character(unlist(taxeco1)),
              ncol = ncol(taxeco1),
              nrow = nrow(taxeco1))
...
tax_f <- dplyr::bind_rows(id_tax, unid_tax)
...
## enable to merge the "tax_f" where all fungi clusters are present with their reference from SCATA with "taxeco_id" the dataset manually IDed + FunGuild attribution. 
...
taxeco2$OTU_ID <- c(taxeco2$OTU_ID[1:998],  taxeco2$Reference[999:length(taxeco2$Reference)]) # merging reference and otu_id 
