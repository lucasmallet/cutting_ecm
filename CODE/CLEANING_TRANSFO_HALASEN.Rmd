...
otu_001 <- subset(otu_001_transf, !rowSums(otu_001_transf) == 0) # deleting clusters with 0 

nrow(otu_001_transf)
nrow(otu_001)
nrow(otu_001_transf) - nrow(otu_001) # 1930 clusters with relative abundance < 1% = suppressed -> 299 conserved (all fungal type)
...
rownames(otu_t)
otu_rarefied <- as.data.frame(rrarefy(otu_t, min(rowSums(otu_t)))) # apply rarefaction on nb of total reads per sample (all fungal types)
