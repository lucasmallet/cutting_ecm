...
physeq_12 <- subset_samples(physeq, physeq@sam_data$year=="2012") %>% clean_pq(remove_empty_samples = T,remove_empty_taxa = T, clean_samples_names = T)
physeq_18 <- subset_samples(physeq, physeq@sam_data$year=="2018") %>% clean_pq(remove_empty_samples = T,remove_empty_taxa = T, clean_samples_names = T)

orient <- subset_samples(physeq, physeq@sam_data$specification_value %in% c("N", "S")) %>% clean_pq(remove_empty_samples = T,remove_empty_taxa = T, clean_samples_names = T)
stream <- subset_samples(physeq, physeq@sam_data$specification_value %in% c("UP", "LO")) %>% clean_pq(remove_empty_samples = T,remove_empty_taxa = T, clean_samples_names = T)
```

*VAR TRANSFO FOR ANALYSIS*

```{r transfo variables}
hala_sam <- lapply(physeq@sam_data, as.factor) %>% as.data.frame()
sample_data(physeq)$year_factor <- as.factor(sample_data(physeq)$year)
```

```{r transposing otu table in physq object for hill_pq}
physeq_otu_t <- t(physeq@otu_table)
physeq_sam <- physeq@sam_data
physeq_tax <- physeq@tax_table
physeq_t <- phyloseq(physeq_otu_t,physeq_sam,physeq_tax)
```

```{r grouping by factor of insterest}
hala_grp_12 <- merge_group(physeq_12, group = "treatment_edge",)
hala_grp_18 <- merge_group(physeq_18, group = "treatment_edge",)
```
