...
# clust all
raw_clust <- read.table("raw_clusters.txt",header=T,sep="\t",na.strings=T,dec=".") %>% dplyr::arrange(desc(raw_clust[,2],))
...
# subset guilds
ecm <- subset(tax_table,tax_table$primary_lifestyle=="ectomycorrhizal") %>% dplyr::arrange(desc(ecm[,2],))
sapr <- subset(tax_table,tax_table$primary_lifestyle!="ectomycorrhizal")
```
# DATA RUN
```{r results mm}
colnames(run_data)
...
## nb reads
run_data[3,2]
raw_total_read <- sum(raw_clust$Cluster_Size); raw_total_read
